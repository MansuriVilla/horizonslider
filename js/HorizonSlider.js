...
function CustomSlider(container, options) {
    if (!(container instanceof HTMLElement)) {
        console.error("CustomSlider: Provided container is not an HTMLElement.", container);
        return;
    }
    ...
    this._handleMouseLeaveBound = () => { if (this.options.autoplay) this.initAutoplay(); };
...
CustomSlider.prototype._initializeSlider = function () {
    ...
    if (!this.track) {
        console.error("CustomSlider: '.slider-track' element not found inside the container.", this.container);
        ...
        if (this.slides.length === 0) {
            console.warn("CustomSlider: No '.slide' elements found in the track. Slider may not function as expected.", this.container);
    ...
    if (this.options.enableSlider && this.track && this.slides.length > 0) {
        ...
        console.warn("CustomSlider: Slider functionality is disabled or essential elements are missing for container:", this.container);
    }
};
...
CustomSlider.prototype.setupSlider = function () {
    ...
    if (this.trackerContainer) this.trackerContainer.style.display = this.options.showTracker ? "" : "none";
...
CustomSlider.prototype.updateSlider = function () {
    ...
    if (this.trackerThumb && this.options.showTracker && this.visibleSlides < this.slides.length) {
        ...
        if (trackMovementRange > 0) { // Avoid division by zero
...
CustomSlider.prototype.initEvents = function () {
    ...
    if (this.trackerThumb && this.trackerContainer && this.options.showTracker && typeof Draggable !== 'undefined') {
        new Draggable(this.trackerThumb, {
            ...
            onDrag: () => {
                ...
                if (this.visibleSlides < this.slides.length) {
                    ...
                    const thumbMovementRange = this.trackerContainer.offsetWidth - this.trackerThumb.offsetWidth;
                    const trackMovementRange = Math.abs(this.trackWidth - this.container.offsetWidth);
                    ...
                    if (thumbMovementRange > 0 && trackMovementRange > 0) {
                        ...
                        this.currentIndex = Math.round(Math.abs(targetTrackX) / slideWidthWithGap);
        ...
        console.warn("GSAP's Draggable plugin is not loaded. Tracker thumb will not be draggable.");
    }
...
CustomSlider.prototype.initDragEvents = function () {
    ...
    this.track.addEventListener("touchstart", this._handleTouchStartBound, { passive: true });
    this.track.addEventListener("touchmove", this._handleTouchMoveBound, { passive: false }); // touchmove cannot be passive if you want to prevent default scroll
    this.track.addEventListener("touchend", this._handleTouchEndBound, { passive: true });
    this.track.addEventListener("mousedown", this._handleTouchStartBound, { passive: true });
    this.track.addEventListener("mousemove", this._handleTouchMoveBound, { passive: false });
    this.track.addEventListener("mouseup", this._handleTouchEndBound, { passive: true });
    this.track.addEventListener("mouseleave", this._handleTouchEndBound, { passive: true }); // End drag if mouse leaves track
};
...
CustomSlider.prototype.handleTouchEnd = function () {
    ...
    const swipeThreshold = slideWidthWithGap * 0.2; // Threshold for swipe detection

    let targetIndex = this.currentIndex;

    if (Math.abs(diffX) > swipeThreshold) {
        // If it was a significant swipe, calculate target index based on swipe direction and distance
        const currentTrackX = new DOMMatrixReadOnly(window.getComputedStyle(this.track).transform).m41;
        targetIndex = Math.round(Math.abs(currentTrackX) / slideWidthWithGap);
    } else {
        // If not a significant swipe, snap to the nearest slide based on current track position
        const currentTrackX = new DOMMatrixReadOnly(window.getComputedStyle(this.track).transform).m41;
        targetIndex = Math.round(Math.abs(currentTrackX) / slideWidthWithGap);
    }

    // Ensure targetIndex is within valid bounds
    targetIndex = Math.max(0, Math.min(targetIndex, this.slides.length - this.visibleSlides));
...
CustomSlider.prototype.initAutoplay = function () {
    if (this.options.autoplay && this.slides.length > this.visibleSlides) { // Only autoplay if there's more than one visible slide to scroll
        this.stopAutoplay();
        const delay = this.options.autoplay.delay || 3000;
        this.autoplayTimer = setInterval(() => {
            const maxIndex = this.slides.length - this.visibleSlides;
            if (maxIndex <= 0) { // If all slides are visible, stop autoplay
                this.stopAutoplay();
                return;
            }
            if (this.options.loop) {
...
CustomSlider.prototype._handleResize = function () {
    ...
    if (this.trackerThumb && this.options.showTracker && typeof Draggable !== 'undefined') {
...
function advanceSlider(config) {
    ...
    if (typeof Draggable === "undefined") {
        console.warn("GSAP Draggable not found. Attempting to load Draggable.min.js...");
        const script = document.createElement("script");
        script.src = "https://cdnjs.cloudflare.com/ajax/libs/gsap/3.11.4/Draggable.min.js";
        script.onload = () => {
            ...
            if (typeof gsap !== "undefined") {
                ...
                console.error("GSAP core library must be loaded before Draggable. Please include gsap.min.js.");
            }
        };
        script.onerror = () => console.error("Failed to load Draggable script from CDN.");
...
function _initializeSliders(config) {
    for (const selector in config) {
        ...
        elements.forEach((el) => {
            ...
            if (el.customSliderInstance && typeof el.customSliderInstance.destroy === 'function') {
...
if (typeof window !== "undefined") {
    ...
    console.warn("CustomSlider and advanceSlider not attached to window: Non-browser environment detected.");
}