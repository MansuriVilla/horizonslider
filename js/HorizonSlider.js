...
HorizonSlider.prototype._applyCenterStyles = function () {
  this.slides.forEach((slide, index) => {
    slide.style.transition = 'transform 0.3s ease, opacity 0.3s ease, scale 0.3s ease';
...
HorizonSlider.prototype.calculateSlideWidth = function () {
  ...
  if (this.options.center) {
    ...
    this.trackWidth = this.slideWidth * this.slides.length + gap * Math.max(0, this.slides.length - 1);
  } else {
    this.slideWidth = (containerWidth - gap * (this.visibleSlides - 1)) / this.visibleSlides;
    this.trackWidth = this.slideWidth * this.slides.length + gap * (this.slides.length - 1);
  }
...
HorizonSlider.prototype.updateSlider = function () {
  ...
  if (this.options.center && this.slides.length > 1) {
    ...
    const lastSlideCenter = centerOffset - (this.slides.length - 1) * (this.slideWidth + slideSpacing);
        ...
        if (this.visibleSlides < this.slides.length) {
          ...
          if (thumbMovementRange > 0 && trackMovementRange > 0) {
            ...
            if (this.options.center) {
              ...
              this.currentIndex = Math.round(progress * (this.slides.length - 1));
...
HorizonSlider.prototype._handleNavClick = function (direction) {
  this.stopAutoplay();
  const maxIndex = this.options.center ? this.slides.length - 1 : this.slides.length - this.visibleSlides;
...
HorizonSlider.prototype.handleTouchMove = function (e) {
  ...
  if (this.options.center) {
    ...
    const centerOffset = containerWidth / 2 - this.slideWidth / 2;
    const firstSlideCenter = centerOffset;
    const lastSlideCenter = centerOffset - (this.slides.length - 1) * slideWidthWithGap;
    
    // Apply friction to edges for a "rubber band" effect
    if (newTranslateX > firstSlideCenter) {
      newTranslateX = firstSlideCenter + (newTranslateX - firstSlideCenter) * 0.3;
    } else if (newTranslateX < lastSlideCenter) {
      newTranslateX = lastSlideCenter + (newTranslateX - lastSlideCenter) * 0.3;
    }
  } else {
